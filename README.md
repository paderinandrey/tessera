...
|---|---|---|---|---|---|
| sentence (80 chars) | 0.0 ms | 49 ms | 76 ms | 120 ms | 145 ms |
| paragraph (1 200 chars, one chunk) | 0.6 ms | 528 ms | 641 ms | 1 138 ms | 1 372 ms |
| document (6 000 chars, several chunks) | 3.3 ms | 3 757 ms | 3 626 ms | 5 345 ms | 9 008 ms |

Per-layer figures are medians. The p95 column is the number REQ-38 asks about; on a
developer machine under load it carries contention as well as detector behaviour — the
document row measured 5 566 ms p95 on a quieter run against 9 008 ms here, while the
medians moved by under 5%. Treat the medians as the stable signal and the p95 as an upper
bound until these run on dedicated hardware.
...
> would have reported 145 ms and hidden that, which is why the harness uses a size ladder.
>
> The per-tier split says something useful too: three quasi-identifier labels cost about as
> much as eleven Article 9 labels. The price is paid per inference pass, not per label, so
> adding categories to an existing tier is nearly free while adding a tier is not.
>
