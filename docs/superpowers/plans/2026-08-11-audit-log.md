...
- Produces:
  ...
  - `pub(crate) trait Sink: Send` with `write_line` and `sync`, and `#[cfg(test)] pub(crate) fn Audit::with_sink(sink: Box<dyn Sink>, salt: [u8; 32]) -> Audit`

**Why a sink rather than a bare `File`.** The behaviour this whole slice turns on is that a journal which cannot be written refuses the request. That has to be tested, and it cannot be provoked through the filesystem: a read-only directory does not disturb an already-open descriptor, and neither does removing or chmod-ing the file. Injecting the sink is the smallest change that makes the guarantee testable instead of asserted by inspection. It is one indirection on a path that already takes a mutex.
...
mod tests {
    ...
    fn a_sink_that_cannot_write_surfaces_the_error() {
        assert!(failing_audit_for_tests().append(r#"{"a":1}"#, true).is_err());
    }
...
/// A journal whose writes land but whose `sync` fails — an fsync failure on an
/// otherwise working filesystem. `failing_audit_for_tests` cannot stand in for
/// it: a sink that fails everything takes the outcome line down too, leaving
/// nothing to read back. Returns the path the lines land in.
#[cfg(test)]
pub(crate) fn fsync_failing_audit_for_tests() -> (Audit, PathBuf) {
    struct FsyncFailingSink(File);

    impl Sink for FsyncFailingSink {
        fn write_line(&mut self, line: &str) -> std::io::Result<()> {
            self.0.write_line(line)
        }
        fn sync(&mut self) -> std::io::Result<()> {
            Err(std::io::Error::other("the disk rejected the flush"))
        }
    }

    let dir = std::env::temp_dir().join(format!("tessera-audit-{}", random_id()));
    ...
    let file = OpenOptions::new()
        ...
        .expect("opens");
    (
        Audit::with_sink(Box::new(FsyncFailingSink(file)), [7u8; SALT_BYTES]),
        path,
    )
}
```
...
```rust
    ...
    async fn a_failed_masked_write_refuses_rather_than_returning_ok() {
        ...
        let record = Record::new(Arc::new(failing_audit_for_tests()), "openai", "/v1/chat/completions");
        let error = record.masked().await.expect_err("a journal that cannot be written refuses");
    ...
    #[tokio::test]
    async fn a_failed_masked_write_leaves_upstream_false() {
        // The field an auditor reads first. A journal that could not record
        // the request refuses it before the provider is called, so claiming
        // bytes left the perimeter would be false evidence. Needs a sink whose
        // sync fails while its writes land — an fsync failure on a working
        // filesystem — because a sink that failed everything would take the
        // outcome line down with it and leave nothing to read.
        let (audit, path) = fsync_failing_audit_for_tests();
        let record = Record::new(Arc::new(audit), "openai", "/v1/chat/completions");
        record.masked().await.expect_err("an fsync that fails refuses");
        drop(record);

        let outcome = lines(&path)
            .into_iter()
            .find(|line| line["event"] == "outcome")
            .expect("the outcome line still lands");
        assert_eq!(outcome["upstream"], false);
    }
...
impl Record {
    ...
    pub async fn masked(&self) -> Result<(), AuditError> {
        ...
        // Only after the write landed. `upstream` claims that bytes left the
        // perimeter, and a failed `masked` refuses the request before the
        // provider is called — so setting it while building the line would
        // put a false claim in the one field an auditor reads first.
...
Run: `cd gateway && cargo test audit::`
Expected: PASS, twenty-one tests — the nine from Task 2 and the twelve above.
...
```rust
    ...
    async fn a_journal_that_cannot_be_written_refuses_before_the_provider() {
        ...
        assert!(!body.contains('/'), "no filesystem detail reaches the client");
        assert!(
            upstream.received_requests().await.expect("recorded").is_empty(),
...
| A write failure refuses with 503 and calls nobody | 2 (sink), 3 (unit), 4 (end to end) |
